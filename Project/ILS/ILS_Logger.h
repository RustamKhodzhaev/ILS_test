...
public:
	...
	// Регистрация информационного сообщения, которую надо переопределить.
	virtual void logOut(const Msg& msg, const LogId& id) const = 0;
	// Регистрация предупреждения (warning) и не фатальной ошибки, которую надо переопределить.
	virtual void wrnOut(const Msg& msg, const LogId& id) const = 0;
	// Регистрация фатальной ошибки, которую надо переопределить.
