...
std::string BaseLogger::title() const {
	...
	if (show_info & 4) {
		long fDurationMilli = long(std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - start_time).count());		
		sprintf(s, "% 8.2f ", double(fDurationMilli));
		res += s;
	}

	return res;
}
// Для кажного типа сообщения задается отдельная функция, которая 
